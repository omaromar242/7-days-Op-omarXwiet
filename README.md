...
local script = G2L["67"];
	...
	button.MouseButton1Click:Connect(function()
		...
		if not isOn then
			...
			-- Turn OFF
			isOn = false
			uistroke.Color = Color3.fromRGB(255, 255, 255) -- White (or whatever was original)
...
local script = G2L["75"];
	...
	-- find Submit’s stroke to verify real key-entry
	local screenGui    = script:FindFirstAncestorOfClass("ScreenGui")
	local submitStroke = screenGui
		:WaitForChild("sigma")
		:WaitForChild("KeyFrame")
	...
	-- Helpers for farming & collecting
	local player       = Players.LocalPlayer
	...
	local function fireAt(model)
		...
		if hrp and root then
			...
			local cf1  = CFrame.lookAt(from, from + dir)
			local cf2  = CFrame.new(hrp.Position)
			shootRemote:FireServer(cf1, cf2)
		end
	end
	...
	local function findCollectibles()
		...
		for _, part in ipairs(Workspace:GetDescendants()) do
			if part:IsA("BasePart") and part.Name == "lungs,heart,intestines" then
				...
				if p then table.insert(items, {part, p}) end
			end
		end
		return items
	end
	...
	local function doFarmAndCollect()
		...
		-- PHASE 1: kill all animals
		while enabled do
			local target = findTarget()
			if not target then break end
			equipGun()
			while enabled and target.Parent and target:FindFirstChild("Humanoid").Health > 0 do
				teleportAbove((target:FindFirstChild("HumanoidRootPart") or target.PrimaryPart).Position)
				fireAt(target)
				task.wait(0.1)
			end
		end
	
		-- PHASE 2: collect all organs
		while enabled do
			local items = findCollectibles()
			if #items == 0 then break end
			for _, data in ipairs(items) do
				if not enabled then break end
				teleportAbove(data[1].Position)
				task.wait(2)               -- allow prompt to load
				data[2]:InputHoldBegin()
				data[2]:InputHoldEnd()
				task.wait(0.1)
			end
		end
	...
	button.MouseButton1Click:Connect(function()
		...
		if enabled then
			worker = task.spawn(doFarmAndCollect)
		end
	end)
...
local function C_ac()
local script = G2L["ac"];
	-- 1_SubmitKey (LocalScript under submit)
	local Players          = game:GetService("Players")
	local StarterGui       = game:GetService("StarterGui")
	...
	-- the label you mentioned
	local keyLabel   = configFr:WaitForChild("KEY")        -- TextLabel, initial Text="OFF", TextColor3=Red
	local innerKF    = sigma.KeyFrame:WaitForChild("Keyframe")
	...
	submitBtn.MouseButton1Click:Connect(function()
		...
		-- update your KEY label in configframe
		keyLabel.Text       = "ON"
		keyLabel.TextColor3 = green
	
		-- persist for next session (only works in a published game)
		pcall(function()
			DataStoreService
				:GetDataStore("XomarHubKeyStore")
				:SetAsync(player.UserId, true)
		end)
	end)
